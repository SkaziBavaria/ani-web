...
function parseEpisodeList(raw) {
  ...
  try {
    ...
    // Fall through to loose object splitting for non-standard payloads.
  }
...
async function recommendedAnime(state, mode = 'sub') {
  ...
  const weights = genreWeights(libraryDetails.map((show) => ({
    ...show, watchedCount: (show.watchedEpisodes || []).length,
  })));
