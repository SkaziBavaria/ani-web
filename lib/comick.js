...
const { getChapterPagesByTitle: getMangaTownPages } = require('./mangatown');
const { resolvedTitleMatchesAny } = require('./title-match');

const DEFAULT_PAGE_RESOLVERS = [
  { name: 'Weeb Central', resolve: getChapterPagesByTitle },
  { name: 'MangaPill', resolve: getMangaPillPages },
  { name: 'MangaDex', resolve: getMangaDexPages },
  { name: 'MangaTown', resolve: getMangaTownPages },
];
const DEFAULT_PAGE_RESOLVER_NAMES = DEFAULT_PAGE_RESOLVERS.map((resolver) => resolver.name);
let pageResolvers = DEFAULT_PAGE_RESOLVERS;
...
async function getChapterPages(id, chapter) {
  ...
  for (const resolver of pageResolvers) {
    try {
      const result = await resolver.resolve(names, chapter);
...
function setPageResolversForTests(resolvers) {
  pageResolvers = Array.isArray(resolvers)
    ? resolvers.map((resolver, index) => (
      typeof resolver === 'function' ? { name: `Resolver ${index + 1}`, resolve: resolver } : resolver
    ))
    : DEFAULT_PAGE_RESOLVERS;
}
