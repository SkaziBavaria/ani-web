...
async function migrateShowToHianime(show) {
  if (!show?.id || show.hianimeId) return { changed: false };
  ...
  return {
    changed: true,
      value: {
        ...
        provider: 'hianime',
        hianimeId: match.id,
        legacyAnidbId: show.legacyAnidbId || show.id,
...
async function migrateLibraryToHianime(state, { limit = 25, save = null } = {}) {
  ...
  const shows = Object.values(state.shows || {})
    .filter((show) => !show.hianimeId && (!show.providerMigrationAttemptedAt || now - Date.parse(show.providerMigrationAttemptedAt) >= 24 * 60 * 60_000))
