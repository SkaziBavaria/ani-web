...
function parseEpisodes(raw, slug) {
  const text = unescapeHtml(raw).replace(/\\n/g, ' ');
  ...
  for (const block of text.split(/ep-item/i).slice(1)) {
    ...
    items.push({ id: Number(id), number: normalizeEpisode(number), title: decodeEntities(block.match(/data-title=["']([^"']*)["']/i)?.[1] || '') });
  }
  const seen = new Set();
...
async function searchAnime(query) {
  ...
  return parseSearchResults(html).map((item, index) => ({ ...item, index: index + 1, title: item.title || item.name, provider: 'hianime', hianimeId: item.id }));
}
...
function parseChart(html, range) {
  const period = { '1': 'day', '7': 'week', '30': 'month' }[String(range)];
...
async function popularAnime(range = '0') {
  if (String(range) === '0') return browseAnime('/most-popular');
  return parseChart(await fetchHiAnime('/home'), range).map((item) => ({ ...item, title: item.title || item.name, provider: 'hianime', hianimeId: item.id }));
}
...
function parseShowDetails(html, slug) {
  ...
  const nativeName = decodeEntities(html.match(/<h2[^>]+class=["'][^"']*film-name[^"']*["'][^>]*data-jname=["']([^"']+)/i)?.[1] || '').trim()
    || field('Japanese');
  return {
    id: slug, provider: 'hianime', hianimeId: slug, name, sourceName: name, title: name, description,
...
function parseSupportedServers(raw, mode) {
  ...
  return [...unescapeHtml(raw).matchAll(/<[^>]+class=["'][^"']*server-item[^"']*["'][^>]*>/gi)]
    .map(([tag]) => ({
    ...
    .filter((server) => server.language === language && server.name === 'ZokoAnime' && server.hash)
    .slice(0, 4);
}
  ...
  for (const server of serversForLanguage.slice(0, 4)) {
    try {
      ...
      if (!links.length) {
        ...
        return [{ url: masterUrl, quality: null, referrer, provider: 'hianime', server: server.name }];
      }
      return links.map((link) => ({ ...link, server: server.name, subtitle: config.subtitles?.find((item) => item.default)?.src || null }));
    } catch (error) {
      lastError = error;
    }
  }
