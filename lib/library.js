...
const {
  searchAnime, getCachedShowDetails, hasNextSeason, normalizeRelatedShows, animeTitleScore,
} = require('./anidb');
...
const PRESERVED_DETAIL_FIELDS = new Set([
  ...
  'relatedShows', 'relations', 'nextSeason', 'hasNextSeason', 'episodeTitles', 'episodeDates', 'episodes', 'episodeCount', 'latestEpisode', 'episodeCounts', 'lastActivityAt',
]);
...
function pickReleaseWatchMatch(query, results) {
  return (results || [])
    .map((show) => ({ show, score: animeTitleScore(query, show) }))
    .filter(({ score }) => score >= 90)
    .sort((left, right) => right.score - left.score)[0]?.show || null;
}

async function matchHiAnime(state, id, providerId, query) {
  const existing = state.shows[id];
  ...
  const candidate = results.find((item) => item.hianimeId === providerId);
  if (!candidate) throw Object.assign(new Error('Selected HiAnime title is no longer available'), { status: 422 });
  return mergeShow(state, {
    ...existing,
    hianimeId: candidate.hianimeId,
    provider: 'hianime',
    legacyAnidbId: existing.legacyAnidbId || existing.id,
...
async function refreshShow(state, show) {
  ...
  if (show.hianimeId) {
    const details = await animeProvider.details(state, show, mode, { force: true });
    assertSafeShowRefresh(show, details);
    const merged = mergeShow(state, {
      ...show,
      ...details,
      provider: 'hianime',
      hianimeId: show.hianimeId,
