...
const { fetchPublic } = require('./proxy');
const { pathInside } = require('./static');
...
function cacheChapter(id, language, chapter, result) {
  ...
  return writeManifest(id, language, chapter, {
    ...
    verifiedTitleMatch: result.verifiedTitleMatch === true,
    catalogProvider: result.catalogProvider || '',
    catalogRequestId: result.catalogRequestId || '',
    catalogHid: result.catalogHid || '',
    catalogSlug: result.catalogSlug || '',
...
async function downloadChapter(id, language, chapter, result) {
  ...
  for (const [index, page] of (result.pages || []).entries()) {
    const response = await fetchPublic(page.url, { headers: { 'user-agent': USER_AGENT, referer: result.referrer || 'https://weebcentral.com/' } }, 45_000);
  ...
  return writeManifest(id, language, chapter, {
    ...
    resolvedTitle: result.resolvedTitle || '', verifiedTitleMatch: result.verifiedTitleMatch === true,
    catalogProvider: result.catalogProvider || '', catalogRequestId: result.catalogRequestId || '',
    catalogHid: result.catalogHid || '', catalogSlug: result.catalogSlug || '',
    uploadDate: result.uploadDate || null,
  });
}
...
function localPage(id, language, chapter, number) {
  ...
  const dir = chapterDir(id, language, chapter);
  const filename = path.basename(String(page.filename));
  const file = path.resolve(dir, filename);
  if (!pathInside(dir, file)) return null;
...
function isVerifiedRemoteManifest(manifest, id, names, now = Date.now()) {
  // MangaDex At-Home addresses expire; stale remote fallback is not an offline download.
  if (manifest?.sourceName === 'MangaDex') {
    const age = now - Date.parse(manifest.updatedAt || '');
    if (!Number.isFinite(age) || age < 0 || age >= 25 * 60 * 1000) return false;
  }
  return manifest?.downloaded !== true
    && manifest?.verifiedTitleMatch === true
    && manifest?.catalogProvider === 'comick'
    && String(manifest?.catalogRequestId || '') === String(id || '')
    && Boolean(manifest?.pages?.length)
    && Boolean(manifest?.resolvedTitle)
    && resolvedTitleMatchesAny(manifest.resolvedTitle, names);
}

module.exports = {
  cacheChapter, readManifest, presentPages, downloadChapter, listDownloads, cachedChapterDates,
  localPage, deleteDownload, isVerifiedRemoteManifest,
};
