...
const PRESERVED_MANGA_DETAIL_FIELDS = new Set([
  ...
  'countryOfOrigin', 'chapters', 'chapterCount', 'latestChapter', 'lastChapterDate', 'relations', 'lastActivityAt',
]);
...
function assertSafeMangaRefresh(existing, details) {
  ...
  // ComicK payloads are already bound to the requested slug/hid in fetchComicPayload.
  // Primary titles can legitimately switch between English and romanized aliases.
  if (!candidate || /^(?:under maintenance|service unavailable|bad gateway|gateway timeout|access denied|internal server error|just a moment\.{0,3})$/i.test(candidate.trim())) {
    throw new Error('Manga refresh returned invalid metadata');
  }
