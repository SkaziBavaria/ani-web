...
// Maintenance reference: keiyoushi/extensions-source src/en/mangapill (Apache-2.0).
...
function parseChapterImages(html) {
  const pages = [];
  const seen = new Set();
  const pictures = String(html || '').matchAll(/<picture\b[^>]*>([\s\S]*?)<\/picture>/gi);
  for (const picture of pictures) {
    const tag = picture[1].match(/<img\b[^>]*>/i)?.[0] || '';
    const source = tag.match(/\sdata-src\s*=\s*["']([^"']+)["']/i)?.[1];
    if (!source) continue;
