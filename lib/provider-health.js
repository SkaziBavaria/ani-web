...
function recordProviderFailure(provider, error, { retryAt = null, now = Date.now() } = {}) {
  ...
  if (error instanceof Error) {
    const labels = { anidb: 'AniDB', hianime: 'HiAnime', comick: 'ComicK', mangadex: 'MangaDex', weebcentral: 'WeebCentral', mangapill: 'MangaPill', mangatown: 'MangaTown' };
    error.publicFailure = {
      provider,
      mediaMode: ['anidb', 'hianime'].includes(provider) ? 'anime' : 'manga',
...
const RETIRED_PROVIDERS = new Set(['anidb']);

function visibleProviderHealth() {
  return Object.fromEntries(
    Object.entries(providerHealth()).filter(([provider]) => !RETIRED_PROVIDERS.has(provider)),
  );
}
