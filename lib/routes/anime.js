...
const { recommendedAnime } = require('../anime-recommendations');
...
async function handleAnimeRoutes(req, res, url) {
  ...
  if (req.method === 'GET' && url.pathname === '/api/search') {
    ...
    const results = animeProvider.bindCatalogToLibrary(readState(), await animeProvider.search(query));
    return sendJson(res, 200, { results });
  }

  if (req.method === 'GET' && url.pathname === '/api/hianime/search') {
    const query = requiredString({ query: url.searchParams.get('q') }, 'query', { maxLength: 200 });
  ...
  if (req.method === 'GET' && url.pathname === '/api/popular') {
    ...
    const results = animeProvider.bindCatalogToLibrary(readState(), await animeProvider.popular(range));
    return sendJson(res, 200, { results });
  }

  if (req.method === 'POST' && url.pathname === '/api/sequels') {
    const body = await readBody(req);
    const mode = body.mode || readState().settings.mode;
    const ids = Array.isArray(body.ids) ? body.ids : [];
    const byId = await animeProvider.summaries(readState(), ids, mode);
    const sequels = {};
    byId.forEach((summary, id) => {
      sequels[id] = {
        id: summary.id,
        name: summary.name,
        status: summary.status || '',
        episodeCount: summary.episodeCount || 0,
        latestEpisode: summary.latestEpisode || null,
      };
    });
    return sendJson(res, 200, { sequels });
  }
  ...
  if (req.method === 'GET' && episodeMatch) {
    ...
    try {
      const details = await animeProvider.details(state, { id, provider: url.searchParams.get('provider') }, mode);
  ...
  if (req.method === 'GET' && detailsMatch) {
    ...
    const cachedBefore = freshCacheEntry(state, cacheKey);
    try {
      const details = await animeProvider.details(state, { id, provider: url.searchParams.get('provider') }, mode, { includeRelations: true });
      if (touchShow(state, id, details)) saveState(state);
      return sendJson(res, 200, {
        show: presentShow({ ...state.shows[id], ...details }),
        cache: cacheMetadata(cachedBefore, { cached: Boolean(cachedBefore) }),
      });
    } catch (err) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/track') {
    ...
    const show = mergeShow(state, {
      ...
      archived: false,
      ...(body.provider === 'hianime' && body.hianimeId ? { provider: 'hianime', hianimeId: body.hianimeId } : {}),
    });
    saveState(state);
  ...
  const hianimeMatch = url.pathname.match(/^\/api\/shows\/([^/]+)\/hianime$/);
  if (req.method === 'POST' && hianimeMatch) {
    const id = decodeURIComponent(hianimeMatch[1]);
    const body = await readBody(req);
    const hianimeId = requiredString(body, 'hianimeId', { maxLength: 200 });
    const state = readState();
    const query = body.query ? requiredString(body, 'query', { maxLength: 200 }) : '';
    const show = await matchHiAnime(state, id, hianimeId, query);
    saveState(state);
    return sendJson(res, 200, { show: presentShow(show) });
  }
  ...
  if (req.method === 'PATCH' && showMatch) {
    ...
    if (Object.hasOwn(body, 'dismissedNextSeasonId')) {
      patch.dismissedNextSeasonId = body.dismissedNextSeasonId
        ? requiredString(body, 'dismissedNextSeasonId', { maxLength: 200 })
        : '';
    }
    if (Object.hasOwn(body, 'sequelAlertsMuted')) patch.sequelAlertsMuted = Boolean(body.sequelAlertsMuted);
    if (Object.hasOwn(body, 'mode')) {
      const mode = normalizeMode(body.mode);
      const details = await animeProvider.details(state, id, mode);
