...
const { listDownloads: listMangaDownloads, deleteDownload: deleteMangaDownload } = require('../manga-cache');
const { startBackgroundTask, downloadConcurrencyLimit } = require('../jobs');
...
async function queueDownload(state, details, episode, mode, quality) {
  ...
  }, { resolvePlayback: (options) => animeProvider.playback(readState(), details, options) });
...
async function handleDownloadRoutes(req, res, url) {
    ...
    return sendJson(res, 200, {
      ...
      downloads: Object.fromEntries(
        Object.entries(downloads)
          ...
          .map(([key, record]) => [key, downloadStatus(record)])
      ),
      mangaDownloads: Object.values(state.mangas || {}).flatMap((manga) =>
        listMangaDownloads(manga.id, 'sub').map((item) => ({
          ...item,
          mangaId: manga.id,
  ...
  if (req.method === 'DELETE' && url.pathname === '/api/downloads') {
    ...
    for (const [key, record] of Object.entries(state.downloads || {})) {
      ...
      if (record.file || isDownloadBusy(record.status)) deleted += 1;
      removeDownloadFiles(record);
      state.downloads[key] = markRecordDeleted(record);
    }
    for (const manga of Object.values(state.mangas || {})) {
      for (const item of listMangaDownloads(manga.id, 'sub')) {
        if (deleteMangaDownload(manga.id, 'sub', item.chapter)) deleted += 1;
      }
    }
    saveState(state);
  ...
  if (req.method === 'POST' && url.pathname === '/api/download') {
    ...
    const details = await animeProvider.details(state, body, mode);
  ...
  if (req.method === 'POST' && url.pathname === '/api/download-season') {
    ...
    saveState(state);
    return sendJson(res, 200, { queued, concurrency: downloadConcurrencyLimit() });
  }
  ...
  if (req.method === 'GET' && downloadFileMatch) {
    ...
    if (error) return sendError(res, ...error);
    await streamDownloadFile(req, res, record.file.path);
    return;
  }
