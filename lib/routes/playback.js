...
function publicPlaybackError(error, provider = 'anidb') {
  ...
  if (status) return { status, message: `${provider === 'hianime' ? 'HiAnime' : 'AniDB'} unavailable (HTTP ${status})` };
...
module.exports = { handlePlaybackRoutes, publicPlaybackError };
