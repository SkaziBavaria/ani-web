...
function touchShow(state, id, details) {
  ...
  if (details.id !== id) throw new Error('Anime refresh returned the wrong identity');
  mergeShow(state, { ...state.shows[id], ...details, id, lastCheckedAt: new Date().toISOString() });
  return true;
}
