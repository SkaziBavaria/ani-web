...
// Shared HTTP transport. Legacy AniDB exports are retained for compatibility.

const { spawn } = require('child_process');
const { ANIDB_ORIGIN, CURL_IMPERSONATE } = require('./config');
const {
  providerForUrl, recordProviderSuccess, recordProviderFailure,
...
function looksLikeUpstreamErrorPage(body) {
  const title = String(body || '').match(/<title[^>]*>\s*([^<]+?)\s*<\/title>/i)?.[1] || '';
  return /^(?:under maintenance|service unavailable|bad gateway|gateway timeout|access denied|internal server error|just a moment\.{0,3})$/i.test(title.trim());
}

const looksLikeMaintenancePage = looksLikeUpstreamErrorPage;
...
async function fetchWebText(url, {
  ...
  try {
    ...
    if (firstError?.exitCode !== 35 || !canUseNativeFallback(binary)) {
      ...
      recordProviderFailure(provider, firstError);
      throw firstError;
    }
    try {
      ...
      try {
        ...
        recordProviderFailure(provider, retryError);
        throw retryError;
      }
    }
  }
  if (looksLikeCloudflareChallenge(body)) {
    const hint = isPlainCurlBinary(binary)
      ...
      : '';
    const error = new Error(`Blocked by upstream protection when fetching ${absolute}.${hint}`);
    recordProviderFailure(provider, error);
    throw error;
  }
  if (looksLikeUpstreamErrorPage(body)) {
    const error = new Error(`Upstream returned an error page when fetching ${absolute}`);
    recordProviderFailure(provider, error);
    throw error;
  }
